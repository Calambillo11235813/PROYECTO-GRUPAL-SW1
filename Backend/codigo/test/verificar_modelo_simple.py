#!/usr/bin/env python
"""
Script simplificado para verificar el modelo usando test_samples.
No requiere Django completamente configurado, solo el detector.
"""
...
# Intentar importar el detector directamente
try:
    from codigo.modelos.production_code_detector import get_detector
except ImportError as e:
    print(f"Error al importar el detector: {e}")
    print("\nIntentando configurar Django...")
    ...
    try:
        ...
        print(f"Error al configurar Django: {e2}")
        print("\nPor favor, asegúrate de que:")
        print("1. El entorno virtual esté activado")
        print("2. Django esté instalado: pip install django")
...
def analizar_archivo(ruta_archivo, esperado, detector):
...
def main():
    ...
    try:
        ...
        print(f"{Colors.RED}Error al cargar el detector: {e}{Colors.RESET}")
        return
    ...
    for archivo in archivos_ia:
        ...
        if ruta.exists():
            resultado = analizar_archivo(ruta, 'IA', detector)
            if resultado:
    ...
    for archivo in archivos_humano:
        ruta = test_samples_dir / archivo
        if ruta.exists():
            resultado = analizar_archivo(ruta, 'HUMANO', detector)
            if resultado:
                confianza = resultado.get('confidence', 0.0)
