# Tests moved to `texto/tests/` package. Kept empty to avoid duplicate discovery.

import unittest
import torch

from .predictor import TextoPredictor
...
class FakeModel:
	...
	def __call__(self, **inputs):
		...
		# Construct logits so softmax -> [1-prob, prob]
		p = self.prob_ia
		# To get softmax probability p, logits can be [log(1-p), log(p)] but softmax of [a,b] gives exp(a)/(...)
		# Simpler: choose logits that produce desired softmax approximately
...
class TextoPredictorTests(unittest.TestCase):
	...
	def test_predict_long_text_chunks(self):
		...
		self.assertIn('prediccion', res)
		self.assertIn('probabilidad_ia', res)
		# With model prob_ia=0.7, prediction should be IA
		self.assertEqual(res['prediccion'], 'IA')
